...
new node with the given weight and
NULL left and right pointers. */
node *
newNode (int weight, int i)
{
  node *nodee = new node ();
  nodee->weight = weight;
  nodee->ue = i;
  nodee->left = NULL;
  nodee->right = NULL;
  nodee->height = 1; // new node is initially
      // added at leaf
  return (nodee);
}
...
// Recursive function to insert a weight
// in the subtree rooted with node and
// returns the new root of the subtree.
node *
insert (node *nodee, int weight, int i)
{
  /* 1. Perform the normal BST insertion */
  if (nodee == NULL)
    return (newNode (weight, i));

  if (weight < nodee->weight)
    nodee->left = insert (nodee->left, weight, i);
  else if (weight > nodee->weight)
    nodee->right = insert (nodee->right, weight, i);
  else // Equal weights are not allowed in BST
    return nodee;
  ...
  // Left Left Case
  if (balance > 1 && weight < nodee->left->weight)
    return rightRotate (nodee);

  // Right Right Case
  if (balance < -1 && weight > nodee->right->weight)
    return leftRotate (nodee);

  // Left Right Case
  if (balance > 1 && weight > nodee->left->weight)
    {
      nodee->left = leftRotate (nodee->left);
      return rightRotate (nodee);
    }
  ...
  if (balance < -1 && weight < nodee->right->weight)
    {
      nodee->right = rightRotate (nodee->right);
      return leftRotate (nodee);
    }
...
preOrder (node *root)
{
  if (root != NULL)
    {
      cout << root->weight << "(ueID-" << root->ue << ",RBs-" << root->RBs << ") ";
      preOrder (root->left);
      preOrder (root->right);
    }
}
...
void
allocateRBs (node *&root, double x)
{
  if (root == NULL)
    return;
  ...
  root->RBs = x;
  if (root->left != NULL)
    allocateRBs (root->left, root->left->weight * x / root->weight);
  if (root->right != NULL)
    allocateRBs (root->right, root->right->totalWeightUptoLeaf * x / root->weight);
}

// Driver Code
int
assignTotalWight (node *&nodee)
{
  if (nodee == NULL)
    return 0;
  nodee->totalWeightUptoLeaf =
      nodee->weight + assignTotalWight (nodee->left) + assignTotalWight (nodee->right);
  return nodee->totalWeightUptoLeaf;
}

double
totalPriortisedRBs (node *root)
{
  if(root==NULL) return 0;
  if(root->ue<=noOfPriotized) return root->RBs + totalPriortisedRBs(root->left)+ totalPriortisedRBs(root->right);
  else return totalPriortisedRBs(root->left)+ totalPriortisedRBs(root->right);
}

void
updateRBs (node *&nodee, double factor,vector<int> &v)
{
  if (nodee == NULL)
    return;
  ...
  updateRBs (nodee->right, factor,v);
}
...
vector<int> useOfOwnBuiltFunction(){
  node *root = NULL;
  ...
  for (int i = 0; i < noOfUE; i++)
    {
      int k = 2.5 * noOfUE;
      int a;
      if (i < noOfPriotized)
        a = rand () % (k) + 2.5 * noOfUE * noOfUE / (noOfUE - noOfPriotized);
      else
        a = rand () % (k);
      a++;
      while (mep[a] != 0)
  ...
  for (int i = 0; i < noOfUE; i++)
    {
      root = insert (root, v[i], i + 1);
    }

  int d = assignTotalWight (root);
  allocateRBs (root, 100);

  double totalPriortisedRbs = totalPriortisedRBs (root);
  double factor = 25 / totalPriortisedRbs; // this is fixed number which means we are assigning at max 13 rbs to one node

  vector<int> rbsToId(noOfUE);
  updateRBs (root, factor,rbsToId);
...
vector<int> mappingOfIdWithRbs = useOfOwnBuiltFunction();
...
RrFfMacScheduler::DoSchedUlTriggerReq (const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
  ...
  if (m_harqOn == true) // if some error is found then this function is call and again that ue's has to send buffer status report
    {
  ...
  for (it = m_ceBsrRxed.begin (); it != m_ceBsrRxed.end (); it++)
    ...
    map<int,int> rntiWithId;
    int countOwn = 1;
    for (it = m_ceBsrRxed.begin (); it != m_ceBsrRxed.end (); it++)
    {
      rntiWithId[(*it).first] = countOwn;
      countOwn++;
    }
  ...
  uint16_t rbPerCurNumFlows;
  rbPerCurNumFlows = rbPerFlow;
  ...
  do
    {
      ...
      // if((*it).first<=4 || (*it).first==9 || (*it).first==10)
      // {
      //   if( rbAllocated + prRbPerFlow <= m_cschedCellConfig.m_ulBandwidth){
      rbPerFlow = mappingOfIdWithRbs[rntiWithId[(*it).first]];
        // }
      // }

      int id = rntiWithId[(*it).first];
      // cout<<id<<endl;
      ...
      // useOfOwnBuiltFunction();
      std::cout<<"Rnti "<<(*it).first<<" rbPerFlow "<<rbPerFlow<<std::endl;
      // for(int i = 1;i<countOwn;i++){
      //   cout<<mappingOfIdWithRbs[i]<<" ";
      // }
      cout<<endl;
      ...
      while ((!allocated)&&((rbAllocated + rbPerFlow - m_cschedCellConfig.m_ulBandwidth) < 1) && (rbPerFlow != 0))
        {
          ...
          for (uint16_t j = rbAllocated; j < rbAllocated + rbPerFlow; j++)
            {
              if (rbMap.at (j) == true) // this means if this particular rb is true this means we cant allocate this rb as it is not free 
                {
                  free = false;
                  break;
                }
          ...
          if (rbAllocated + rbPerFlow - 1 > m_cschedCellConfig.m_ulBandwidth)
            {
              ...
              if (rbPerFlow < 3)
                {
                  // terminate allocation
                  rbPerFlow = 0;                 // iske baad kya hoga find out
                }
            }
        }
      if (!allocated)
